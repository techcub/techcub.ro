...
if (!out) {
  console.error(
    'verify-build: no build output found — expected dist/client/index.html or dist/index.html'
  );
  process.exit(1);
}
...
for (const [name, minBytes] of Object.entries(REQUIRED_ASSETS)) {
  ...
  if (size < minBytes)
    failures.push(`${name} is only ${size} bytes (expected at least ${minBytes})`);
}
...
if (failures.length === 0)
  note(`assets: ${Object.keys(REQUIRED_ASSETS).length} favicon files, all valid`);
...
process.stdout.write('verify-build: output looks good\n');
