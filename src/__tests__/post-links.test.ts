...
const post = (id: string, uid: string | undefined, title: string, locale = 'en') => ({
  id,
  data: { locale, uid, title },
});
...
describe('buildUidIndex', () => {
  it('indexes posts by uid then locale, deriving slug and title', () => {
    const index = buildUidIndex([post('en/it-support-guide', 'support-guide', 'Support Guide')]);
    expect(index.get('support-guide')?.get('en')).toEqual({
  ...
  it('throws when two posts in the same locale claim the same uid', () => {
    expect(() => buildUidIndex([post('en/a', 'dup', 'A'), post('en/b', 'dup', 'B')])).toThrow(
      /Duplicate canonical id "dup"/
    );
  });
});
